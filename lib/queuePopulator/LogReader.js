...
class LogReader {
    ...
    _setupMetricsProducer(done) {
        this._mProducer = new MetricsProducer(this.zkConfig,
            this.mConfig.topic);
    ...
    _processPublishEntries(batchState, done) {
        ...
        return async.each(Object.keys(entriesToPublish), (topic, done) => {
            ...
            // TODO: Refactor this
            if (topic === this.mConfig.topic) {
                return async.series([
                    done => this._setupMetricsProducer(done),
                    done => this._mProducer.send(topicEntries, done),
                ], err => {
                    if (err) {
                        this.log.error('error publishing metrics-specific '
