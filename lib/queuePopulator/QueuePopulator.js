...
class QueuePopulator {
    ...
    open(cb) {
        async.parallel([
            ...
            done => this._loadExtensions(done),
        ], err => {
            if (err) {
    ...
    close(cb) {
        return async.parallel([
            ...
            done => this._unloadExtensions(done),
        ], cb);
    }
    ...
    _loadExtensions(done) {
        this._extensions = [];
        async.each(Object.keys(this.extConfigs), (extName, done) => {
            ...
            ext.setup(err => {
                if (!err) {
                    this.log.info(`${index.name} extension is ready`);
                    this._extensions.push(ext);
                } else {
                    this.log.error('an error occurred loading ' +
    ...
    _unloadExtensions(done) {
        async.each(this._extensions, (ext, done) => ext.shutdown(done),
                   done);
    }
