...
class BackbeatConsumer extends EventEmitter {
    ...
    */
    constructor(config) {
        ...
        const { kafka, ssl, topic, groupId, queueProcessor,
        ...
        // XXX HACK: force a refresh of metadata periodically to
        // overcome bugs when a kafka client times out and fails to
        // recover its connection
        setInterval(() => {
            this._log.debug('kicking periodic refreshMetadata');
            this._consumer.client.refreshMetadata([this._topic], error => {
                if (error) {
                    this._log.error('KafkaClient refresh metadata error',
                                    { error });
                }
            });
        }, 30000);
